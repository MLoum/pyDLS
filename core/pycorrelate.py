...
def correlate_whal(t_stamp_a_true, t_stamp_b_true, lags, B=10, is_normalize=True, is_auto_cor=True):
    """
    lags in tick
    :param t_stamps_a:
    :param lags:
    ...
    """
    G = np.zeros(lags.size)
    coeff_a = np.ones(t_stamp_a_true.size)
    coeff_b = np.ones(t_stamp_b_true.size)
    # print(datetime.datetime.now(), " Start with :", t_stamp_a_true[0])
    t_stamp_a = np.copy(t_stamp_a_true)
